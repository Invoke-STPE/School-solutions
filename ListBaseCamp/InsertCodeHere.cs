...
namespace ListBaseCamp
{
    public class InsertCodeHere
    {
        public void MyCode()
        {
            ...
            foreach(int element in listOfInt) {
            ...
            int smallestNum = listOfInt[0];
            for (int element = 0; element < listOfInt.Count; element++) 
            {
                if (smallestNum > listOfInt[element]) 
                {
                    smallestNum = listOfInt[element];
                }
            }
            System.Console.WriteLine($"Smallest number is: {smallestNum}");

            // [(maybe) DIFFICULT]
            // Case 8: Add code that sorts the content of the list.
            // Tip: Think before you code...
            listOfInt.Sort(); // Sorts the list, and assigns to the old one.
            foreach (int element in listOfInt) 
            {
                System.Console.WriteLine(element);
            }
